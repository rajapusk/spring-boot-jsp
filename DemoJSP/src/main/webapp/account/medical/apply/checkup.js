var MedicalCheckup = function(){
	...
	var bindComponent = function(){
		...
		.jqxFileUpload({ 
			...
			uploadUrl:  Common.HOST + '/medical/uploadFile' 
		...
		$("#dvRefundablePFLoan").jqxForm('getComponentByName', 'hospitalName').on('change', function(event){
			formData.hospitalName = event.args.value;
		});
	...
	var submitLoan = function(sType){
		var sURL = Common.HOST + '/medical/create'
	...
	var getLoanData = function(){
		...
		var fields = [
			...
			{field: 'hospitalName', name: 'hospitalName'}, 
	...
	var calculateAmount = function(){
		...
		if(formData.claimAmount == null || formData.claimAmount == 0 || formData.claimDate == null || formData.claimDateDays > 30 
			|| formData.invoiceDate == null || formData.invoiceDateDays > 30 || formData.age < 45){
